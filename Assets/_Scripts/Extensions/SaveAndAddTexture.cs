...
{
    public static void SaveTextureToFile(this Texture2D texture, ObjectSettings asset)
    {
        var folderPath = FolderPath.NewAssetFolder(asset.name);
        
        if (!System.IO.Directory.Exists(folderPath))
            System.IO.Directory.CreateDirectory(folderPath);

        // save texture to file in the folder we just created
        var fullSavePath = GetSavePath(asset.name);
